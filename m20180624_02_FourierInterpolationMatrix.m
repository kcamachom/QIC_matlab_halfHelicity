function M = m20180624_02_FourierInterpolationMatrix(N, x)
%function t = m20180624_02_FourierInterpolationMatrix(fk, x)

% N = number of grid points for the input data.
% x = Vector of x-values where interpolant is to be evaluated.
% It is assumed that the period is 2pi.
...
if rem(N,2) == 0                  
  ...
  M = diag(1./(D*w)) * D * diag(w);            % Evaluate interpolant as 
