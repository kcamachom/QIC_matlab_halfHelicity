function [varphi,phi,iota,alpha_RMS,curvature, torsion, n_for_alpha, sigma_max, elongation] = mGarrenBoozerOmnigenity_minimizer_halfHelicity(nfp,R0c,R0s,Z0c,Z0s,dk,dk_c)%,B0_1,B0_2)
%function mGarrenBoozerOmnigenity_1layerIt_Frenet_FieldPeriods_alpha(RZvalues)
%function mGarrenBoozerOmnigenity_1layerIt_Frenet_FieldPeriods_alpha()

% See "20190701-01 Omnigenity construction with single layer iteration.docx"

% N_phi = Number of grid points in phi
N_phi_array = 201;%%%1001
%N_phi_array = [31];
%N_phi_array = [7, 15];
...
%nfp = 1; % For now only nfp=1 is allowed.
...
B0_as_a_function_of_varphi = @(varphi) 1 + B0_1 * cos(varphi) + B0_2*cos(2*varphi);
...
%     for j= 1:size(phi)
%         if phi(j) < pi/nfp
%             d(j) = curvature(j).*sqrt(0.25./B0(j));
%         elseif phi(j) > pi/nfp
%            d(j) = -curvature(j).*sqrt(0.25./B0(j)); 
%         end
%     end
    d = curvature.*sqrt(dk./B0);
    for k = 1:length(dk_c)
        d = d + curvature.*dk_c(k).*cos(k*nfp*varphi);
    end
    
%      for j= 1:size(phi)
%          if phi(j) < pi/nfp
%             d(j) = curvature(j).*sqrt(dk./B0(j)) + curvature(j).*dk_c(k).*cos(k*nfp*varphi(j));% + curvature(j).*0.3.*sin(nfp*varphi(j))   ;
%         elseif phi(j) > pi/nfp
%            d(j) = curvature(j).*sqrt(dk./B0(j)) + curvature(j).*dk_c(k).*cos(k*nfp*varphi(j)); %+ curvature(j).*0.35.*cos(nfp*varphi(j));%+ curvature(j).*0.3.*sin(nfp*varphi(j)) ; 
%         end
% end  
...
% %%%Save B file for comparison with booz_xform output
% B_filename = ['Scans_3FP_3/B_B1_Objective_',num2str(nfp),'FP_A',num2str(aspect_ratio),'_Z1_',num2str(Z0s(2)),'_Z2_',num2str(Z0s(3)),'.mat'];
% B0_O = B0;
...
% hold on 
% plot(phi,R0+r*(R1c*cos(theta_testVMEC(3))+R1s*sin(theta_testVMEC(3))))
...
% 
% vmec_filename = ['./Scans_2FP_halfHelicity_axis/',vmec_filename_base,'QI_',num2str(nfp),'FP_A',num2str(aspect_ratio),'_R4_',num2str(R0c(4)),'_Z3_',num2str(Z0s(3)),'_Z4_',num2str(Z0s(4))]; 
% %'_B01_',num2str(B0_1),'_B02_',num2str(B0_2)];
...
% fprintf(fid,'FTOL_ARRAY  = 1e-17 1e-15 1e-13 1e-11\n');
...
for j_theta = 1:N_theta
    costheta = cos(theta_1D(j_theta));
    sintheta = sin(theta_1D(j_theta));
    X1 = X1c * costheta + X1s * sintheta;
    Y1 = Y1c * costheta + Y1s * sintheta;
